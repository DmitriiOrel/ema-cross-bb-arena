...
class ScalpelStrategy(BaseStrategy):
    ...
    async def add_signal(self, df: DataFrame):
        ...
        condition_buy = cross_up & (df["Close"] > df["bb_upper"])
        condition_sell = cross_down & (df["Close"] < df["bb_lower"])
        df["TotalSignal"] = 0
    ...
    async def start(self):
        if not self.account_id:
            try:
                accounts = (await client.get_accounts()).accounts
                if not accounts and settings.sandbox:
                    await client.open_sandbox_account()
                    accounts = (await client.get_accounts()).accounts
                if not accounts:
                    logger.error("No accounts available. Stopping strategy.")
                    return
